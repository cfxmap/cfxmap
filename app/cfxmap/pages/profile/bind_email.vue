<template>
	<view class="container" :class="themeClass">
		...
		<view class="header">
			...
			<text class="title">{{ i18n.bindEmailTitle || 'Bind Email' }}</text>
		</view>
			...
			<view class="captcha-modal" @click.stop>
				...
				<view class="captcha-modal-img-block">
					<view class="captcha-modal-img-card" @click="refreshImageCaptcha">
						<image
							v-if="modalCaptchaImage"
							class="captcha-modal-image"
							:src="modalCaptchaImage"
							mode="aspectFit"
						/>
		...
		<view class="tips-card" v-if="currentEmail">
			<text class="tips-title">{{ i18n.bindEmailStepsTitle || 'Two-Step Verification Required' }}</text>
			<text class="tips-desc">{{ i18n.bindEmailStepsDesc || 'Verify your current email first, then verify the new email.' }}</text>
		</view>

		<!-- Step 1: Verify Current Email -->
		<view class="step-card" v-if="step === 1">
			<text class="step-title">{{ i18n.step1VerifyOld || 'Step 1: Verify Current Email' }}</text>
			
			<view class="input-row readonly" v-if="currentEmail">
				<text class="input-field">{{ i18n.currentEmail || 'Current Email' }}: {{ currentEmailMasked }}</text>
			</view>

			<view class="input-row">
				<input class="input-field" type="text" :placeholder="i18n.enterOldCode || 'Enter verification code'" v-model="oldCode" placeholder-class="placeholder-text" />
				<view
					class="send-code-btn"
					:class="{ 'send-code-btn--disabled': countdownOld > 0 }"
					@click="sendOldCode"
				>
					<text class="send-code-text">{{ labelSendOld(i18n) }}</text>
				</view>
			</view>

			<button class="btn-primary" @click="verifyOldEmail">
				<text class="btn-text">{{ i18n.verifyCurrentEmail || 'Verify Current Email' }}</text>
			</button>
		</view>

		<!-- Step 2: Verify New Email -->
		<view class="step-card" v-if="step === 2">
			<text class="step-title" v-if="currentEmail">{{ i18n.step2VerifyNew || 'Step 2: Verify New Email' }}</text>
			<text class="step-title" v-else>{{ i18n.bindEmailTitle || 'Bind Email' }}</text>
			
			<view class="input-row">
				<input class="input-field" type="text" :placeholder="i18n.enterNewEmail || 'Enter new email address'" v-model="newEmail" placeholder-class="placeholder-text" />
			</view>

			<view class="input-row">
				<input class="input-field" type="text" :placeholder="i18n.enterNewCode || 'Enter new email code'" v-model="newCode" placeholder-class="placeholder-text" />
				<view
					class="send-code-btn"
					:class="{ 'send-code-btn--disabled': countdownNew > 0 }"
					@click="sendNewCode"
				>
					<text class="send-code-text">{{ labelSendNew(i18n) }}</text>
				</view>
			</view>

			<button class="btn-primary" @click="bindNewEmail">
				<text class="btn-text">{{ i18n.completeBinding || 'Complete Binding' }}</text>
			</button>
		</view>
	</view>
</template>
...
<script>
	...
	import emailCodeCaptchaDual from '@/mixins/email-code-captcha-dual.js'
	import NavBackBtn from '@/components/nav-back-btn.vue'

	export default {
		components: { NavBackBtn },
		mixins: [emailCodeCaptchaDual],
		data() {
			return {
				step: 1,
				currentEmail: '', // 从用户信息获取
				oldCode: '',
				newEmail: '',
				newCode: ''
			}
		...
		computed: {
			...
			currentEmailMasked() {
				...
				const parts = this.currentEmail.split('@');
				if (parts.length !== 2) return this.currentEmail;
		...
		methods: {
			...
			sendOldCode() {
				this.onTapSendOldCode(this.currentEmail, this.i18n)
			},
			onCaptchaModalConfirm() {
				this.confirmCaptchaModalSend(this.currentEmail, this.newEmail, this.i18n)
			},
			async verifyOldEmail() {
				...
				try {
					...
					await api.verifyEmailCode(this.currentEmail, this.oldCode)
					uni.hideLoading()
					this.resetCaptchaAfterOldEmailVerified()
					this.step = 2
				} catch (e) {
					uni.hideLoading()
				}
			...
			sendNewCode() {
				this.onTapSendNewCode(this.newEmail, this.i18n)
			},
			async bindNewEmail() {
				...
				try {
					uni.showLoading({ title: this.i18n.binding })
					await api.bindEmail(this.newEmail, this.newCode)
					uni.hideLoading()
					this.resetCaptchaAfterNewEmailVerified()
					uni.showToast({ title: this.i18n.bindSuccess, icon: 'success' })
