...
public class CaptchaController {
    ...
    public void emailCodeImpl(String email) {
        ...
        if (sender != null) {
            sender.sendCfxmapVerifyCode(email, code, Constants.CAPTCHA_EXPIRATION);
            return;
        }
        try {
            CfxmapVerifyCodeMail.send(email, code, Constants.CAPTCHA_EXPIRATION);
        } catch (Exception e) {
            log.error("验证码邮件发送异常 => {}", e.getMessage());
            throw new ServiceException("验证码邮件发送异常: " + e.getMessage());
        }
    }

    /**
     * 生成验证码（管理端 /auth/code；App 与统一网关常用 /api/auth/code）
     */
    @GetMapping({"/auth/code", "/api/auth/code"})
    ...
    public CaptchaVo getCodeImpl() {
        ...
        int charLen = ObjectUtil.defaultIfNull(captchaProperties.getCharLength(), 4);
        CodeGenerator codeGenerator;
        if ("math".equals(captchaType)) {
            codeGenerator = new MathGenerator(captchaProperties.getNumberLength(), false);
        } else if ("number".equals(captchaType)) {
            codeGenerator = new RandomGenerator("0123456789", charLen);
        } else {
            codeGenerator = new RandomGenerator(charLen);
        }
